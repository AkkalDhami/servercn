...
export default function PackageManagerTabs({ command = "" }: { command: string }) {
  return (
    <Tabs defaultValue="npm" className="dark:code-theme my-6 max-w-[900px] overflow-auto rounded-md bg-editor border-0">
      <TabsList className="dark:code-theme bg-editor">
        ...
        {Object.keys(managers).map(m => (
          <TabsTrigger
            key={m}
            value={m}
            className="dark:data-[state=active]:bg-editor data-[state=active]:shadow-none flex items-center gap-2 font-medium text-neutral-400 data-[state=active]:bg-editor data-[state=active]:text-black dark:data-[state=active]:border-transparent dark:data-[state=active]:text-white">
