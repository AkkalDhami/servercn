---
title: Changelog
description: Latest updates and announcements.
---


## September 2026

#### 25 September, 2026 - Docs Redesign
- Refactored the documentation UI with **shadcn Typeset** for a cleaner and more consistent reading experience.
- Improved typography, spacing, headings, and content hierarchy across the documentation.
- Updated the docs styling to better match the overall servercn design system.

---

#### 22 September, 2026 - Express.js Response Formatter
- Refactored the <Code>response-formatter</Code> component to use the <Code>ApiResponse</Code> class.
- Added command and manual installation instructions.

<PackageManagerTabs command="npx servercn-cli@latest add response-formatter" />

[Read more](/docs/express/components/response-formatter)

---

## August 2026

#### 28 August, 2026 - Nest.js Response Formatter Component
- Added the <Code>response-formatter</Code> component for Nest.js.

<PackageManagerTabs command="npx servercn-cli@latest add response-formatter" />

[Read more](/docs/nestjs/components/response-formatter)

---

#### 06 August, 2026 - Next.js Health Check Component
- Added the <Code>health-check</Code> component for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add health-check" />

[Read more](/docs/nextjs/components/health-check)

---

## July 2026

#### 25 July, 2026 - Express.js Stateless Auth Foundation

- Added the <Code>stateless-auth</Code> foundation for Express.js.

#### 1. MongoDB + Mongoose

<PackageManagerTabs command="npx servercn-cli@latest init stateless-auth --db=mongodb --orm=mongoose" />

<PackageManagerTabs command="npx servercn-cli@latest init stateless-auth --db=mysql --orm=drizzle" />

[Read more](/docs/express/foundations/stateless-auth)

---

#### 22 July, 2026 - Express.js Stateful Auth Foundation

- Added the <Code>stateful-auth</Code> foundation for Express.js.

#### 1. MongoDB + Mongoose

<PackageManagerTabs command="npx servercn-cli@latest init stateful-auth --db=mongodb --orm=mongoose" />

[Read more](/docs/express/foundations/stateful-auth)

---

#### 19 July, 2026 - Express.js Hybrid Auth Foundation

- Added the <Code>hybrid-auth</Code> foundation for Express.js.

#### 1. MongoDB + Mongoose

<PackageManagerTabs command="npx servercn-cli@latest init hybrid-auth --db=mongodb --orm=mongoose" />

#### 2. MySQL + Drizzle

<PackageManagerTabs command="npx servercn-cli@latest init hybrid-auth --db=mysql --orm=drizzle" />

#### 3. PostgreSQL + Drizzle

<PackageManagerTabs command="npx servercn-cli@latest init hybrid-auth --db=postgresql --orm=drizzle" />

[Read more](/docs/express/foundations/hybrid-auth)

---

#### 08 July, 2026 - Express.js Hybrid Auth with MySQL(Drizzle) Blueprint

- Added the <Code>hybrid-auth</Code> blueprint for Express.js.

<PackageManagerTabs command="npx servercn-cli@latest add bp hybrid-auth" />

[Read more](/docs/express/blueprints/hybrid-auth-mysql)

---

#### 02 July, 2026 - Next.js Mongodb(Prisma) Provider

- Added the <Code>mongodb-prisma</Code> provider for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add pr mongodb-prisma" />

[Read more](/docs/nextjs/providers/mongodb-prisma)

---

## June 2026

#### 29 June, 2026 - Next.js PostgreSQL(Drizzle & Neon) Provider

- Added the <Code>pg-drizzle-neon</Code> provider for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add pr pg-drizzle-neon" />

[Read more](/docs/nextjs/providers/pg-drizzle-neon)

---

#### 27 June, 2026 - Next.js PostgreSQL(Drizzle) Provider

- Added the <Code>pg-drizzle</Code> provider for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add pr pg-drizzle" />

[Read more](/docs/nextjs/providers/pg-drizzle)

---

#### 26 June, 2026 - Next.js MySQL(Drizzle) Provider

- Added the <Code>mysql-drizzle</Code> provider for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add pr mysql-drizzle" />

[Read more](/docs/nextjs/providers/mysql-drizzle)

---

#### 25 June, 2026 - Next.js Mongodb(Mongoose) Provider

- Added the <Code>mongodb-mongoose</Code> provider for Next.js.

<PackageManagerTabs command="npx servercn-cli@latest add pr mongodb-mongoose" />

[Read more](/docs/nextjs/providers/mongodb-mongoose)

---

#### 20 June, 2026 - Express.js Rate Limiter Component

- Added <Code>upstash</Code> and <Code>ioredis</Code> variants for the Rate Limiter component.
- Expanded the component beyond the existing <Code>express-rate-limit</Code> implementation.

<PackageManagerTabs command="npx servercn-cli@latest add rate-limiter" />

[Read more](/docs/express/components/rate-limiter)

---

#### 18 June, 2026 - Next.js File Upload Component(Imagekit)


You can now add a file upload component powered by Imagekit.

<PackageManagerTabs command="npx servercn-cli@latest add file-upload" />

[Read more](/docs/changelog/2026-06-18-nextjs-file-upload-imagekit)

---


#### 16 June, 2026 - Next.js File Upload Component(Cloudinary)

You can now add a file upload component powered by Cloudinary.

<PackageManagerTabs command="npx servercn-cli@latest add file-upload" />

[Read more](/docs/changelog/2026-06-16-nextjs-file-upload-cloudinary)
