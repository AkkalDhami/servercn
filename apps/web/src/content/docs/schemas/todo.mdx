...
- `completed`: Boolean flag indicating if the task is finished.
...
This schema assumes you have the **Auth Domain** installed. The `userId` field acts as a foreign key (PostgreSQL/MySQL) or reference (MongoDB) to the `User` model. This allows you to:
