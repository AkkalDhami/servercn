...
By default, the CLI uses the configuration defined in your <Code children={'servercn.config.json'} /> file (typically initialized during `npx servercn-cli init`). It will automatically detect your project's **Database**, **Architecture**, and **Language**.
...
When no configuration is found, initialize it using <Code children={'npx servercn-cli init'} />.
