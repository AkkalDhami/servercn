...
1.  **Authentication Middleware**: Detailed in the [Verify Auth Middleware](/docs/express/components/verify-auth-middleware) component.
...
<Note text="If your authentication middleware only attaches the user ID (e.g., from a JWT payload), you may need to fetch the full user profile or include the role in the JWT payload." />
...
```ts title="src/middlewares/authorize-role.ts"
