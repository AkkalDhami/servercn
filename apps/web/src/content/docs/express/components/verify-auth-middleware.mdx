...
<Steps>
<Step>Install the prerequisites</Step>
Ensure the following environment variables are defined in <Code>.env</Code>:
...
```ts title="src/middlewares/verify-auth.ts"
...
```ts {2} /verifyAuthentication,/ title="src/routes/user.routes.ts"
