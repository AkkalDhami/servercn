...
```ts title="src/jobs/example.job.ts"
...
Import and call the <Code>initJobs</Code> function in your main entry file (usually server.ts).
...
```ts title="src/jobs/cleanup-refresh-tokens.job.ts"
import cron from "node-cron";
