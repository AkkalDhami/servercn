...
command: npx servercn add github-google-oauth
...
**_👉 Note:_** _You do not need to install any servercn dependencies manually. Installing this component will automatically install all required servercn dependencies._
...
<PackageManagerTabs command="npx servercn add github-google-oauth" />
...
(See `mvc_structure` blocks below for file details)

### 2. Feature Structure

(See `feature_structure` blocks below for file details)

## Limitations & Common Issues

- **Stateless**: Uses `session: false`.
- **Manual Persistence**: You must implement the database saving logic.
- **Redirect URI Mismatch**: Ensure strict matching with provider settings.
