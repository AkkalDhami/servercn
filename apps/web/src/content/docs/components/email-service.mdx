...
Create email service in <Code children="src/services/email.service.ts" />:
...
For feature-based architecture, place service in <Code children="src/modules/email/email.service.ts" />:
...
await sendTextEmail("user@example.com", "Welcome to ServerCN", "Thank you for joining us!");
```
...
export const sendWelcomeEmail = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(STATUS_CODES.OK).json(ApiResponse.success(null, "Welcome email sent successfully"));
});
```
